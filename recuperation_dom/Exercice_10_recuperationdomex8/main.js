...
for (let skills in competences) 
{
    console.log(`${skills}`)
    console.log(competences[skills]);
};

let tabObjet = Object.values(competences);
console.log(tabObjet);

for (let i = 0; i < tabObjet.length; i++) 
{
    tab[i].innerHTML += ` ${tabObjet[i]}`;
    console.log(tab[i]);
};
...
{
    if (parseInt(tabObjet[j]) < 50)
    {
        tab[j].style.backgroundColor = "red";
    }
    else if (parseInt(tabObjet[j]) == 100)
    {
        tab[j].style.backgroundColor = "gold";
        tab[j].style.color = "black"
    }
    else if (parseInt(tabObjet[j]) > 50)
    {
